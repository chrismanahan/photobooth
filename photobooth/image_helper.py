...
class ImageHelper:
	...
	def create_text_image(self, img, text, font_size, position, background_color = None, yPadding = 0):
		drawing = ImageDraw.Draw(img)
		w, h = drawing.textsize(text, font=self.get_font(font_size))
		if background_color is not None:
			pad = 50
			self.create_rectangle(img, w + pad, h + pad, background_color, position, yPadding)
		box = self.createBoundingBox(position, w, h, yPadding)
	...
	## pw = parent width
	## ph = parent height
	def create_button_text_image(self, img, text, font_size, pw, ph, position, yPadding = 0):				
		drawing = ImageDraw.Draw(img)
		coords = self.textPositionForBoundingBox(position, pw, ph, yPadding)
		drawing.text(coords, text, font=self.get_font(font_size), fill=(255,255,255))

	def create_circle_image(self, img, dia, color, position, outline_color = "black", yPadding = 0):
		drawing = ImageDraw.Draw(img)
		bounding_box = self.createBoundingBox(position, dia, dia, yPadding = yPadding)
		drawing.ellipse(bounding_box, fill = color, outline = outline_color)
		
	def create_rectangle(self, img, w, h, color, position, outline_color = "black", yPadding = 0):
		drawing = ImageDraw.Draw(img)
		bounding_box = self.createBoundingBox(position, w, h, yPadding)
		drawing.rectangle(bounding_box, outline_color, color)
	...
	def createBoundingBox(self, position, w, h, yPadding = 0):
		x = 0
		y = 0
	...
	## return coordinates to left align a text drawing under
	## a bounding box
	def textPositionForBoundingBox(self, position, w, h, yPadding = 0):
		box = self.createBoundingBox(position, w, h, yPadding)
		x = box[0][0]
		y = box[1][1]
		return (x, y)
